...
function StarRating({ rating }: { rating: number }) {
  return (
    <span className="flex items-center gap-1 text-xs text-stone-500">
      <svg className="w-3.5 h-3.5 text-gold-300 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.368 2.448a1 1 0 00-.364 1.118l1.286 3.957c.3.921-.755 1.688-1.54 1.118L10 15.347l-3.451 2.778c-.785.57-1.838-.197-1.539-1.118l1.286-3.957a1 1 0 00-.364-1.118L2.564 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69L9.049 2.927z"/></svg>
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const price = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", notation: "compact", maximumFractionDigits: 0 }).format(product.price);
        ...
        <article className="h-full bg-white rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 border border-mint-100/60 flex flex-col">
          ...
          <div className="p-5 flex flex-col flex-1 gap-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-[11px] font-mono text-stone-400 uppercase tracking-wider mb-0.5">{product.brand} · {product.sku}</p>
            ...
            <p className="text-sm text-stone-500 leading-relaxed line-clamp-2 flex-1">{product.shortDescription}</p>
